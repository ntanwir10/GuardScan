...
function complexFunction(x) {
    ...
    it('should run security scan successfully', () => {
      try {
        const output = execSync(
          `node ${path.join(__dirname, '../../dist/index.js')} security --no-telemetry`,
          {
            cwd: tempDir,
            encoding: 'utf-8',
            timeout: 30000,
  ...
  describe('SBOM Generation', () => {
    it('should generate SBOM in SPDX format', () => {
      ...
      try {
        const output = execSync(
          `node ${path.join(__dirname, '../../dist/index.js')} sbom -f spdx -o ${sbomPath} --no-telemetry`,
          {
            cwd: tempDir,
            encoding: 'utf-8',
            timeout: 30000,
  ...
  describe('Config Management', () => {
    it('should initialize config', () => {
      ...
      try {
        execSync(
          `node ${path.join(__dirname, '../../dist/index.js')} init --no-telemetry`,
          {
            cwd: tempDir,
            encoding: 'utf-8',
            timeout: 30000,
  ...
  describe('Status Command', () => {
    it('should show status information', () => {
      try {
        const output = execSync(
          `node ${path.join(__dirname, '../../dist/index.js')} status --no-telemetry`,
          {
            cwd: tempDir,
            encoding: 'utf-8',
            timeout: 30000,
  ...
  describe('LOC Counter', () => {
    it('should count lines of code', () => {
      try {
        const output = execSync(
          `node ${path.join(__dirname, '../../dist/index.js')} run --no-telemetry`,
          {
            cwd: tempDir,
            encoding: 'utf-8',
            timeout: 30000,
