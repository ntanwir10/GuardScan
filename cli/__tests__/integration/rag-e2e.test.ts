...
2. Run the server: \`npm start\`
  ...
  describe("Full RAG Workflow", () => {
    it("should complete the full workflow: index -> search -> chat", async () => {
      ...
      expect(fs.existsSync(path.join(testRepoDir, "src", "auth.ts"))).toBe(
        true
      );
      expect(fs.existsSync(path.join(testRepoDir, "src", "user.ts"))).toBe(
        true
      );
    });
  ...
  describe("Error Handling and Edge Cases", () => {
    ...
    it("should handle files with parsing errors", async () => {
      ...
      fs.writeFileSync(
        invalidFile,
        "function broken( { this is invalid syntax"
      );
