...
describe("Config Lifecycle Integration", () => {
  ...
  it("should complete full config lifecycle: init -> load -> update -> reset", async () => {
    ...
    if (actualConfigDir !== expectedConfigDir) {
      console.warn(`ConfigManager using ${actualConfigDir} instead of ${expectedConfigDir}`);
    ...
    (jest as any).spyOn(inquirer, "prompt").mockResolvedValue({
      mode: "static",
    });
  ...
  it("should handle config operations with debug logging", async () => {
    ...
    const consoleErrorSpy = (jest as any)
      .spyOn(console, "error")
      .mockImplementation();
    const consoleLogSpy = (jest as any)
      .spyOn(console, "log")
      .mockImplementation();
    ...
    const hasDebugOutput = consoleErrorSpy.mock.calls.length > 0 || consoleLogSpy.mock.calls.length > 0;
