...
describe('ConfigManager', () => {
  ...
  describe('load', () => {
    ...
    it.each([
      ...
      ['1', false, false],
      ['null', false, false],
    ])('fails closed for malformed telemetry consent %s', (yamlValue, enabled, offlineMode) => {
      configManager.init();
      const configPath = path.join(configManager.getConfigDir(), 'config.yml');
      fs.writeFileSync(configPath, [
        'provider: none',
        `telemetryEnabled: ${yamlValue}`,
        `offlineMode: ${offlineMode}`,
      ].join('\n'));

      const loaded = configManager.load();
      expect(loaded.telemetryEnabled).toBe(enabled);
      expect(loaded.offlineMode).toBe(false);
    });
...
describe('parseConfig', () => {
  ...
  it('keeps malformed privacy controls fail-closed while rejecting unknown keys', () => {
    const config = parseConfig({
      provider: 'none',
      telemetryEnabled: 'true',
      offlineMode: 'false',
